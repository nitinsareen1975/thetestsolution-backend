...
class PatientController extends Controller
{
    ...
    {
        $sql = "SELECT p.id, p.firstname, p.lastname, p.dob, p.gender, p.street, p.city, p.state, p.zip, p.phone, p.ethnicity, p.pregnent, p.specimen_collection_date, p.specimen_type, p.confirmation_code, p.identifier, p.identifier_type, tt.specimen_site, l.phone as lab_phone, l.licence_number, l.name as lab_name, l.email as lab_email, l.logo, l.date_incorporated, l.facility_id, tt.loinc, tt.name as test_type_name, tt.test_procedure, tt.fi_test_type, (select name from {$this->tableResultTypes} where id=r.id) as result, (select snomed from {$this->tableResultTypes} where id=r.id) as result_snomed, r.result_value, r.created_at as result_date, l.street as lab_street, l.city as lab_city, l.state as lab_state, l.zip as lab_zip, (select name from {$this->tableTestTypeMethods} where id = r.test_type_method_id) as test_type_method FROM {$this->tablePatients} p 
            inner join {$this->tablePricing} lp on lp.id = p.pricing_id 
            inner join {$this->tableTestTypes} tt on tt.id = lp.test_type 
            inner join {$this->tableLabs} l on l.id = p.lab_assigned  
