...
class PatientController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        try {
            ...
            $patients->status = empty($request->input('status')) ? 1 : (boolean)$request->input('status');
            $patients->save();
            return response()->json(['status' => true, 'data' => $patients, 'message' => 'Patient added successfully.'], 201);
        } catch (\Exception $e) {
            return response()->json(['status' => false, 'message' => 'Patient creation failed.'], 409);
        }
    }
    ...
    {
        $query = "SELECT p.*, (SELECT name FROM {$this->tableLabs} WHERE id IN (p.lab_assigned)) as lab_assigned FROM {$this->tablePatients} p WHERE 1=1 "; 
        ...
        $paginationArr = [
            'count' => DB::table($this->tablePatients)->count(),
            'currentPage' => $page,
            'pageSize' => $limit
        ];
